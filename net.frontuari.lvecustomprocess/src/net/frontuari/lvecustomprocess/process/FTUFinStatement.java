...
public class FTUFinStatement extends FTUProcess{
	...
	/** ADDED BY DAVID CASTILLO BY COVENCAUCHO REQUEST     */
	private int 				p_FTU_BPartnerType_ID = 0;
	/** JOIN BPARTNER TYPE                                                  */
	private String 				JoinBPartnerType = "";
	/**	Start Time						*/
	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			else if (name.equals("DateAcct"))
			{
				...
				p_FTU_BPartnerType_ID = ((BigDecimal)para[i].getParameter()).intValue();
			else
		...
		if (p_FTU_BPartnerType_ID != 0) {
			JoinBPartnerType = " LEFT JOIN C_BPartner b ON fa.C_BPartner_ID = b.C_BPartner_ID ";
			m_parameterWhere.append(" AND b.FTU_BPartnerType_ID=").append(p_FTU_BPartnerType_ID);
		}
		//
