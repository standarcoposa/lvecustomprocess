...
public class FTUReplenishReportProduction extends FTUProcess{
	...
	//ADDED By david Castillo 04/05/2021 m_pricelist parameter 
	private int p_M_PriceList_ID = 0;
	private String p_PrintName = null;
	private int p_FTU_ProductGroup_ID = 0;
	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			if (para[i].getParameter() == null)
				...
				p_PrintName = para[i].getParameterAsString().toLowerCase();
			else if (name.equals("FTU_ProductGroup_ID"))
				p_FTU_ProductGroup_ID = para[i].getParameterAsInt();
			else
	...
	private void fillTable (MWarehouse wh) throws Exception
	{
		...
		sql.append(" FROM M_Replenish r");
			...
			sql.append(" JOIN FTU_ProductGroup fpg ON (p.FTU_ProductGroup_ID = fpg.FTU_ProductGroup_ID) ");
		...
		if( p_PrintName != null)
			sql.append(" AND LOWER(fpg.PrintName) like '").append(p_PrintName).append("'");
		if (p_FTU_ProductGroup_ID != 0)
			sql.append(" AND fpg.FTU_ProductGroup_ID = ").append(p_FTU_ProductGroup_ID);
		//product exists in PriceList
		if (p_M_PriceList_ID > 0)
			sql.append(" AND EXISTS(SELECT 1 FROM M_ProductPrice pp JOIN M_PriceList_Version plv on pp.M_PriceList_Version_ID = plv.M_PriceList_Version_ID "
					+ " JOIN M_PriceList pl on plv.M_PriceList_ID = " + p_M_PriceList_ID + ")");
		int no = DB.executeUpdate(sql.toString(), get_TrxName());
		...
		//added by David Castillo 03/09/2021 sumQty from other requisitions
		sql = new StringBuilder("UPDATE T_Replenish t SET ");
			sql.append(" QtyOnHand = QtyOnHand + COALESCE((SELECT SUM(QTY) FROM M_RequisitionLine rl JOIN M_Requisition rq ON rl.M_Requisition_ID = rq.M_Requisition_ID WHERE rq.DocStatus = 'CO' AND rl.C_OrderLine_ID is null AND rl.M_Product_ID = t.M_Product_ID),0)");
			no = DB.executeUpdate(sql.toString(), get_TrxName());
	...
	private void createRequisition()
	{
		...
		for (int i = 0; i < replenishs.length; i++)
		{
				...
				|| requisition.getM_Warehouse_ID() != replenish.getM_Warehouse_ID())
			{
				...
				requisition.set_ValueOfColumn("AD_Role_ID", Env.getAD_Role_ID(getCtx()));
				//	Set Org/WH
				requisition.setAD_Org_ID(wh.getAD_Org_ID());
				requisition.setM_Warehouse_ID(wh.getM_Warehouse_ID());
				requisition.setM_PriceList_ID(p_M_PriceList_ID);
				if (!requisition.save())
					return;
			...
			line.setM_Product_ID(replenish.getM_Product_ID());
			if (replenish.getC_BPartner_ID() > 0)
